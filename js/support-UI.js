(function($){
    ...
    $.feedback = function(options) {
        ...
        var settings = $.extend({
            ...
            tpl: {
                highlighter:    '<div id="feedback-highlighter"><div class="support-logo-header"><h4 class="card-title"><img src="image/customer-service.png" id="support-image" style="height: 70px;width: 60px;"/>SEND SUPPORT</h4></div><hr><h5 class="support-text">Click to scrrenshot with highlighted area and send support.</h5><div class="feedback-buttons text-center"><button id="feedback-overview-back" class="feedback-wizard-back-close btn btn btn-dark btn-fill" style="margin-right: 5px; min-width: 100px;">Back</button><button id="feedback-highlighter-next" class="btn btn-primary btn-fill" style="min-width: 100px;">Next</button><button id="feedback-highlighter-back" class="feedback-back-btn feedback-btn-gray">Back</button></div><div class="feedback-wizard-close"></div></div>',
                overview:       '<div id="feedback-overview"><form method="" action=""><div class="support-logo-header"><h4 class="card-title"><img src="image/customer-service.png" id="support-image" style="height: 70px;width: 60px;"/>SEND SUPPORT</h4></div><hr><div id="feedback-overview-description"><div id="feedback-overview-subject-text" class="form-group"><label>Subject</label></div><div id="feedback-overview-description-text" class="form-group"><label>Description</label></div></div></form><div id="feedback-overview-screenshot"><h3>Screenshot</h3></div><div class="feedback-buttons text-center"><button id="feedback-overview-back" class="btn btn btn-dark btn-fill" style="margin-right: 5px; min-width: 100px;">Back</button><button id="feedback-submit" class="btn btn-primary btn-fill" style="min-width: 100px;">Submit</button></div><div class="feedback-wizard-close"></div></div>'
            },
            onClose:                function() {},
        ...
        if (supportedBrowser) {
            if(isFeedbackButtonNative) {
                // $('.navbar').append('<button class="feedback-btn feedback-btn-gray feedback-btn-native">' + settings.initButtonText + '</button>');
                $('header').append('<button class="nav-item feedback-btn feedback-btn-gray feedback-btn-native">Send Support</button>');
            }
            $(document).on('click.feedback', settings.feedbackButton, function(){
                    ...
                    html2canvas($('body'), {
                        onrendered: function(canvas) {
                            ...
                            if(settings.showDescriptionModal) {
                                ...
                                $('#feedback-overview-subject-text>textbox').remove();
                                $('#feedback-overview-description-text>textarea').remove();
                                $('#feedback-overview-screenshot>img').remove();

                                 $('<input type="textbox" id="feedback-overview-note" name="feedback-overview-subject-text" class="form-control" required />' + $('#feedback-note').val()).insertAfter('#feedback-overview-subject-text label:eq(0)');

                                $('<textarea id="feedback-overview-note" name="feedback-overview-description-text" class="form-control" style="height:90px; required /">' + $('#feedback-note').val() + '</textarea>').insertAfter('#feedback-overview-description-text label:eq(0)');
                ...
                $(document).on('click.feedback', '#feedback-submit', function() {
                    ...
                    var img = $(".feedback-screenshot").attr('src');
                    if ($('#feedback-note').val().length > 0) {
                        ...
                        $.ajax({
                            ...
                            data: data,
                            success: function() {
                                alert('Your Feedback successfully sended..');
